...
function gameover() {
    ...
    isGameOver = false;
    //让背景停下来
    var bg = document.getElementsByClassName('canvas')[0];
    bg.className = 'canvas';
...
function create_pipe() {
    ...
    var topHeight = rand(120, 263);//管道里面上管道的高度值
    var bottomHeight = 490 - 60 - topHeight;//管道里面下管道的高度值
    //创建上下管道
...
function init() {
    ...
    start_btn.addEventListener('touchstart',function () {
        ...
        //通过触屏幕，来调用birdJump方法，来使小鸟上升
        document.addEventListener('touchstart', birdJump, false)
        crashTestTimer = setInterval(judge, 200);
    })
...
game_restart.addEventListener('touchstart',restart)
var conduit_group = document.querySelector(".conduit_group")
